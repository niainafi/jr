  ...
  return (
    <Container className="2xl:max-w-7xl mt-10 mb-10 flex flex-col items-center">
      ...
      <div className="text-center mb-12">
        <h1 className="text-xl md:text-3xl lg:text-4xl font-bold">{`CASQUE BELL LITHIUM MIPS`}</h1>
        <span className="w-[34%] h-2 bg-accent rounded-md mx-auto mt-2"></span> {/* Tiret sous le titre */}
      </div>
      ...
      <div className="flex flex-col items-center gap-8 mb-12">
        ...
        <div className="flex gap-6 mt-6">
          {images.map((img, i) => (
            <div key={i} className="p-2 w-[10rem] h-[10rem] border-2 border-accent rounded-md">
              <Image
                src={img}
                alt={`Détails casque ${i}`}
                width={300}
      ...
      <div className="text-center space-y-6 mb-16">
        <h2 className="text-2xl font-bold inline space-x-1">
          <span>{`BELL LITHIUM MIPS`}</span>
          <span>{`Mais c'est quoi ce MIPS ?`}</span>
        </h2>
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-4 lg:gap-6 mt-24 mb-12 text-center">
        {[1, 2, 4].map((num) => (
          <div key={num} className="border-2 border-accent rounded-sm p-2 mx-auto">
            <Image
              src={`/images/accueil/noir-mat-moto-${num}.webp`}
              alt={`Variante ${num}`}
              width={400}
